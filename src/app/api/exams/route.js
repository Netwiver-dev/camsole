...
export async function POST(req) {
    ...
    try {
        ...
        if (!examData.title || !examData.date || !examData.duration) {
            return new Response(
                JSON.stringify({ error: "Title, date and duration are required" }), { status: 400 }
            );
        }

        if (!examData.questions || examData.questions.length === 0) {
            return new Response(
                JSON.stringify({ error: "At least one question is required" }), { status: 400 }
            );
        }
...
export async function GET(req) {
    ...
    try {
        ...
        if (examId) {
            ...
            if (authResult.user.role === "teacher") {
                if (!exam.createdBy.equals(authResult.user._id)) {
                    return new Response(
                        JSON.stringify({ error: "Unauthorized access to this exam" }), { status: 403 }
                    );
                }
        ...
        if (authResult.user.role === "teacher") {
            const exams = await Exam.find({ createdBy: authResult.user._id }).sort({
                createdAt: -1,
            });
