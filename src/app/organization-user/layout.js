...
export default function RootLayout({ children }) {
    ...
    return ( <
        >
        <
        div className = "flex h-screen overflow-hidden" > { /* Sidebar (Hidden on Mobile) */ } <
        Sidebar / > { /* Main Content Area */ } <
        div className = "flex-1 flex flex-col h-full" > { /* Navbar */ } <
        Navbar / > { /* Dynamic Content */ } <
        main className = "flex-1 overflow-y-auto" > { isLoading ? < SkeletonLoader / > : children } <
        /main> <
